...
\begin{itemize}
    ...
    \item \textbf{Pressure (p)}: \texttt{spaceTimeWindowCoupledPressure} --- mixed Dirichlet/Neumann based on flux direction
...
\begin{itemize}
    ...
    \item At \textbf{inflow faces}, pressure from outside dominates and is prescribed (Dirichlet)
    \item At \textbf{outflow faces}, pressure should adjust freely so only the gradient is prescribed (Neumann)
    \item The BC uses smooth tanh-based blending to transition between these limits
...
oldInternalFaces
{
    ...
    dataDir         "constant/boundaryData";
    phi             phi;
    dirichletWeight 0.8;      // Strength at inflow (0-1)
    neumannWeight   0.8;      // Strength at outflow (0-1)
...
Mixed (Robin) boundary condition for pressure that blends Dirichlet and Neumann conditions based on local flux direction. This is the \textbf{recommended approach} for pressure at extraction boundaries. The parameters are listed in \cref{tab:bc-coupled-pressure-params}.
...
\begin{enumerate}
    ...
    \item Computes local flux at each face to determine flow direction
    \item Calculates blending weight using smooth tanh transition:
    \[
    w = \frac{1}{2}\left(1 - \tanh\left(\frac{\phi_f / A_f}{\sigma}\right)\right)
    \]
    where $\phi_f$ is face flux, $A_f$ is face area, and $\sigma$ is transition width
    \item At \textbf{inflow faces} ($w \to 1$): applies weighted Dirichlet (prescribed pressure)
    \item At \textbf{outflow faces} ($w \to 0$): applies weighted Neumann (prescribed gradient)
\end{enumerate}
...
\texttt{dirichletWeight} & scalar & 0.8 & Dirichlet contribution strength at inflow (0-1) \\
\texttt{neumannWeight} & scalar & 0.8 & Neumann contribution strength at outflow (0-1) \\
\texttt{transitionWidth} & scalar & 0.1 & Sharpness of inflow/outflow transition \\
...
\begin{itemize}
    \item Uses mixed Dirichlet/Neumann based on flux direction
    \item Inflow faces: Dirichlet (prescribed pressure)
    \item Outflow faces: Neumann (prescribed gradient allows natural outflow)
