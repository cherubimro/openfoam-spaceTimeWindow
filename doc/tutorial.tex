...
\begin{tikzpicture}[
    ...
    \node[databox, fill=white, draw=warningorange] at (2,-0.1) {U,p,gradp[0..371]};
    ...
    \node[font=\sffamily\small\bfseries, warningorange] at (10,0.5) {U,p,gradp.dvz[.zstd]};
...
\begin{tikzpicture}[
    ...
    \node[rectangle, rounded corners=4pt, fill=codebg, draw=codeframe, line width=1pt,
        ...
        \textcolor{secondarygreen}{p (pressure)} & Has p + gradp in boundaryData & \texttt{spaceTimeWindowCoupledPressure} \\
...
The library provides three approaches for applying boundary conditions on the extraction boundary (\texttt{oldInternalFaces}):

\subsection{Pressure-Coupled BC --- Recommended}
\label{sec:pressure-coupled-bc}

The \textbf{recommended approach} for accurate reconstruction uses the \texttt{spaceTimeWindowCoupledPressure} boundary condition for pressure, which properly handles the elliptic nature of the pressure equation.
...
\begin{warningbox}
These options (\texttt{-inletOutletBC} and \texttt{-outletDirection}) are mutually exclusive. Using both together produces an error with guidance.
\end{warningbox}
...
\begin{lstlisting}[language=OpenFOAM,caption={Field selection strategy for pressure-coupled BC (RECOMMENDED)}]
...
Note: \texttt{nut} is only needed in \texttt{initialFields} (not in \texttt{fields}) because turbulent viscosity is computed by the turbulence model at runtime and uses \texttt{zeroGradient} BC on \texttt{oldInternalFaces}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \texttt{fluxMagnitude}: $w = w_D + (w_N' - w_D) \cdot \tanh(|\phi_f| / \phi_{\max} / \sigma)$ --- where $w_D$ is \texttt{dirichletWeight} and $w_N' = 1 - w_N$
        \item \texttt{flowDirection}: $w = \frac{1}{2}\left((w_D + w_N') - (w_D - w_N') \cdot \tanh(\phi_f / \phi_{\max} / \sigma)\right)$
    \end{itemize}
