...
\lstdefinelanguage{OpenFOAM}{
    keywords={type, libs, box, outputDir, fields, writeFormat, writeControl,
              ...
              fieldTableName, blendingMode, dirichletWeight, neumannWeight,
              transitionWidth, phi},
    morecomment=[l]{//},
    morecomment=[s]{/*}{*/},
    morestring=[b]",
...
\lstdefinelanguage{bash}{
    keywords={cd, mpirun, pimpleFoam, pimpleFoamPrecise, reconstructPar,
              spaceTimeWindowInitCase, spaceTimeWindowKeygen, decomposePar,
              export, source, wmake},
    morecomment=[l]{\#},
    morestring=[b]",
    morestring=[b]',
...
\hypersetup{
    ...
    bookmarksopen=true,
    bookmarksopenlevel=2,        % Expand to level 2 (sections)
    bookmarksdepth=3,            % Include subsections in bookmarks
...
% Keep floats within their section boundaries
...
\begin{tikzpicture}
    \node[fill=primarylight, rounded corners=4pt, inner sep=8pt, font=\large\sffamily] {
	    \color{primarydark}Version 1.0 \quad|\quad OpenFOAM v2512
    };
\end{tikzpicture}
...
\begin{tikzpicture}
    \node[text width=12cm, align=center, font=\normalsize\color{bodytext}] {
        \textbf{\color{primaryblue}Boundary Data Extraction and Flow Recalculation} ~$\bullet$~
        \textbf{\color{secondarygreen}Time Interpolation} ~$\bullet$~
        \textbf{\color{warningorange}Integrated Zstd Compression} ~$\bullet$~
...
\vspace{2cm}
\centering If this software has been useful in your research, please consider instructions from the \emph{Acknowledging this work} section.
}
...
\begin{itemize}
    ...
    \item Store time-varying boundary conditions efficiently with compact codecs (DVZ/DVZT)
    \item Optionally apply integrated zstd compression for additional $\sim$10$\times$ size reduction
...
\begin{itemize}
    ...
    \item Optional: libzstd for integrated zstd compression (libzstd-dev or libzstd-devel)
    \item Optional: libsodium for encryption support (libsodium-dev or libsodium-devel)
...
Optional dependencies are auto-detected by \texttt{./Allwmake}:
\begin{itemize}
    \item \textbf{libzstd}: Enables integrated zstd compression of DVZ/DVZT files ($\sim$10$\times$ additional reduction)
    \item \textbf{libsodium}: Enables X25519 encryption of boundary data
\end{itemize}

For manual building with optional features:

\begin{lstlisting}[language=bash,caption={Manual build with optional features}]
cd src/spaceTimeWindow
export FOAM_USE_ZSTD=1 FOAM_USE_SODIUM=1 && wmake
...
This section introduces the fundamental concepts behind space-time window extraction and reconstruction in high performance fluid flow simulations. Understanding these concepts is essential before diving into the technical implementation details.
...
\begin{itemize}
    ...
    \item Thousands of timesteps ($\delta t = 1^{-6}$ time stepping)
    \item Multiple field variables (velocity, pressure, turbulence quantities)
    \item Terabytes to petabytes of raw output data
\end{itemize}

Quite often the simulation domain is large enough to allow for the fluid flow to fully develop within much smaller local regions of interest. The European Research Community on Flow, Turbulence and Combustion (ERCOFTAC) database~\cite{ERCOFTAC} contains numerical results and laboratory measurements, and new software implementations must be validated against previously confirmed results (physics remains the same, numerical solvers may change).

Nevertheless, validation is always performed in relation to the physics of the fluid flow, not against bit-exact numerical values from previous simulations.

Storing and post-processing this data is often impractical. When a researcher defines a \emph{region of interest} within a large fluid flow domain, the goal is to \textbf{zoom in} and obtain richer detail about the flow in that region---higher spatial or temporal resolution, more detailed post-processing, or the ability to explore ``what-if'' scenarios with different turbulence models---not to produce a bit-by-bit identical copy of the internal fields from the original simulation. The space-time window approach addresses precisely this need: it extracts only the boundary data required to drive an independent simulation within the region of interest at a later time, and allows the user to change the computational resolution, numerical schemes, or post-processing strategy ``at home'', on modest hardware. The reconstructed flow is physically faithful---it satisfies the same governing equations with the same boundary conditions---but it is not, and need not be, numerically identical to the original (see \cref{sec:concept-why} for a discussion of the discretisation differences).

\subsection{The Space-Time Window Concept}

A \textbf{space-time window}~\cite{Anton2011, Anton2011b, Anton2013} is a bounded region in both space (a 3D box) and time (a time interval) from which we extract the information necessary to reconstruct the flow field. The key insight is that we only need:
...
\begin{tikzpicture}[
    ...
    % Extraction box label (below gray domain for visibility)
    \node[primarydark, font=\sffamily\bfseries] at (4,-0.5) {Extraction Box};

    % Dimensions annotation
    \draw[<->, gray, line width=0.8pt] (2,-1.2) -- (6,-1.2);
    \node[gray, anchor=north] at (4,-1.2) {$L_x$};
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    % Time annotations (centered on arrows with white fill)
    \draw[<->, gray] (-3.8,4.8) -- (-3.8,1.2);
    \node[gray, fill=white, inner sep=2pt, rotate=90, font=\sffamily\footnotesize] at (-3.8,3) {Hours--Days};

    \draw[<->, gray] (11.8,4.8) -- (11.8,1.2);
    \node[gray, fill=white, inner sep=2pt, rotate=90, font=\sffamily\footnotesize] at (11.8,3) {Minutes--Hours};

    % === STORAGE/TRANSFER (drawn last so it appears on top) ===
    \draw[dashed, gray, line width=1pt] (4,-2) -- (4,5.5);
    \node[gray, fill=white, inner sep=3pt, rotate=90] at (4,2) {\textbf{Data Transfer / Archival}};
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    % Output file
    \node[cylinder, shape border rotate=90, draw=warningorange, fill=warninglight, minimum height=0.8cm, minimum width=1.6cm, aspect=0.3] (file) at (0.5,-4) {};
    \node[font=\sffamily\tiny, warningorange] at (0.5,-4) {U,p,gradp};
...
\Cref{fig:mpi-gathering} shows the detailed data flow during the extraction process. The \texttt{spaceTime\-Window\-Extract} function object runs on each MPI process, extracting local contributions that are then gathered and merged into unified output files.
...
\begin{tikzpicture}[
    ...
    \node[anchor=south, font=\sffamily\bfseries, secondarygreen] at (0,7.2) {P0 (Master)};
    \node[databox, fill=white, draw=primaryblue] at (0,6.3) {Local faces: 127};
    \node[databox, fill=white, draw=warningorange] at (0,5.5) {U,p,gradp};
    \node[font=\sffamily\tiny, gray] at (0,4.9) {+ gathering};

    % P1
    \node[procbox, fill=primarylight, draw=primaryblue] (p1) at (4,6) {};
    \node[anchor=south, font=\sffamily\bfseries, primaryblue] at (4,7.2) {P1};
    \node[databox, fill=white, draw=primaryblue] at (4,6.3) {Local faces: 89};
    \node[databox, fill=white, draw=warningorange] at (4,5.5) {U,p,gradp};

    % P2
    \node[procbox, fill=primarylight, draw=primaryblue] (p2) at (8,6) {};
    \node[anchor=south, font=\sffamily\bfseries, primaryblue] at (8,7.2) {P2};
    \node[databox, fill=white, draw=primaryblue] at (8,6.3) {Local faces: 156};
    \node[databox, fill=white, draw=warningorange] at (8,5.5) {U,p,gradp};

    % P3
    \node[procbox, fill=primarylight, draw=primaryblue] (p3) at (12,6) {};
    \node[anchor=south, font=\sffamily\bfseries, primaryblue] at (12,7.2) {P3};
    ...
    % === MPI GATHER ARROWS (straight down into MPI box) ===
    \draw[arrow, secondarygreen, line width=1.5pt] (p0.south) -- (0,3.8);
    \draw[arrow, warningorange, line width=1.5pt] (p1.south) -- (4,3.8);
    \draw[arrow, warningorange, line width=1.5pt] (p2.south) -- (8,3.8);
    \draw[arrow, gray, line width=0.8pt, dashed] (p3.south) -- (12,3.8);
    ...
    \node[font=\sffamily\bfseries, accentpurple] at (6,3) {MPI\_Gatherv: Collect from all ranks};

    % Arrow from MPI to merged
    \draw[arrow, accentpurple, line width=1.5pt] (mpi.south) -- ++(0,-0.4) -| (4,1.8);

    % === MERGED OUTPUT ===
    \node[rectangle, rounded corners=4pt, fill=secondarylight, draw=secondarygreen, line width=1.5pt,
          minimum width=5cm, minimum height=3.2cm] (merged) at (4,0.2) {};
    \node[anchor=north, font=\sffamily\bfseries, secondarygreen] at (4,1.7) {Merged Global Data};
    \node[databox, fill=white, draw=primaryblue] at (4,0.6) {Total faces: 372};
    \node[databox, fill=white, draw=warningorange] at (4,-0.4) {U,p,gradp[0..371]};

    % === OUTPUT FILE ===
    \node[cylinder, shape border rotate=90, draw=warningorange, fill=warninglight,
          minimum height=1.2cm, minimum width=2cm, aspect=0.25, line width=1.5pt] (file) at (11,0.5) {};
    \node[font=\sffamily\small\bfseries, warningorange] at (11,0.5) {.dvz[.zstd]};

    % Arrow from merged to file
    \draw[arrow, secondarygreen, line width=2pt] (merged.east) -- node[above, font=\sffamily\footnotesize] {write} (file.west);

    % === ANNOTATION: What happens each timestep ===
    \draw[decorate, decoration={brace, amplitude=8pt, mirror}, gray, line width=1pt]
        (-2,0.2) -- (-2,8);
    \node[gray, rotate=90, anchor=south, font=\sffamily\footnotesize] at (-2.8,4.1) {Each timestep};
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    \draw[decorate, decoration={brace, amplitude=5pt, mirror}, secondarygreen, line width=1pt]
        (-1.5,1.3) -- (-1.5,2.9);
    \node[secondarygreen, anchor=east, font=\sffamily\footnotesize, align=right] at (-1.8,2.1) {Written\\once};
    ...
    \foreach \x in {3, 5, 7, 11, 13} {
        \node[file, fill=white, draw=warningorange, minimum width=1.2cm, font=\sffamily\tiny] at (\x,1.7) {U,p,gradp};
    }
    \node[font=\sffamily\large, gray] at (9,1.7) {\ldots};
    ...
    % Initial fields size
    \fill[secondarygreen!30] (0,-3) rectangle (2,-2.5);
    \draw[secondarygreen, line width=1pt] (0,-3) rectangle (2,-2.5);
    \node[anchor=west, font=\sffamily\footnotesize] at (2.2,-2.75) {Initial fields: $\mathcal{O}(N_\text{cells})$};

    % Boundary data size (larger bar)
    \fill[warningorange!30] (0,-3.8) rectangle (7,-3.3);
    \draw[warningorange, line width=1pt] (0,-3.8) rectangle (7,-3.3);
    \node[anchor=west, font=\sffamily\footnotesize] at (7.2,-3.55) {Boundary data: $\mathcal{O}(N_\text{faces} \times N_\text{timesteps})$};

    % Note about compression
    \node[anchor=north west, font=\sffamily\footnotesize, text width=12cm, bodytext] at (0,-4.3) {
        $\rightarrow$ Boundary data dominates storage. Use \texttt{dvzt} format with integrated zstd for 300$\times$+ compression.
    };
\end{tikzpicture}
\caption{Data storage structure: Initial fields are written once at $t_0$, while boundary data is accumulated at every timestep. Compression (DVZ/DVZT + integrated zstd) is critical for the boundary data component.}
...
\begin{warningbox}
Boundary data accumulates rapidly during long simulations. For a typical LES with $10^4$ boundary faces and $10^4$ timesteps, uncompressed ASCII data can reach tens of gigabytes. The \texttt{dvzt} compression format reduces this by 30--50$\times$, and with integrated zstd compression the reduction reaches 300$\times$+.
\end{warningbox}
...
The reconstruction closely reproduces the original solution when:
\begin{enumerate}
    ...
    \item The initial fields are stored in binary (lossless) format
\end{enumerate}

\begin{notebox}
Even with exact boundary data and identical solver settings, the subdomain solution is \emph{not} bit-identical to the full-domain solution. This is a fundamental consequence of the finite volume discretisation: at internal faces, fluxes are computed from two neighbouring cell values using the full computational stencil, whereas at boundary faces with prescribed Dirichlet values, the flux uses the boundary value directly and the matrix contribution changes from an off-diagonal coefficient to a source term~\cite{Jasak1996, Moukalled2016}. In particular:
\begin{itemize}
    \item \textbf{TVD/high-resolution convective schemes} compute a limiter ratio from upstream and downstream cell values; at boundary faces the upstream stencil is truncated, altering the limiter behaviour~\cite{Ferziger2020, Jasak1996}.
    \item \textbf{Multigrid pressure solvers} (e.g.\ GAMG) agglomerate cells into coarser levels; the coarsening pattern and the coarse-level matrix structure differ near Dirichlet boundaries compared to the interior~\cite{Ferziger2020}.
    \item \textbf{Gradient reconstruction} (e.g.\ Gauss or least-squares) at boundary-adjacent cells uses the prescribed face value instead of a neighbour cell value, changing the gradient estimate~\cite{Moukalled2016}.
\end{itemize}
These effects are localised: errors are largest in cells adjacent to the extraction boundary and decay rapidly with distance (see \cref{sec:accuracy-comparison}). The \texttt{-ghostCells} option (\cref{sec:ghost-cells}) eliminates this stencil mismatch entirely by keeping the original extraction boundary faces as internal faces, pushing the mismatch to an outer ghost layer where values are overwritten from extracted data.
\end{notebox}

With time interpolation enabled (recommended for practical use), the reconstruction closely approximates the original solution, with additional errors controlled by the interpolation scheme (linear or cubic Catmull-Rom spline).
...
\begin{tikzpicture}[
    ...
    % Title (above the dashed context box)
    \node[anchor=south, font=\sffamily\bfseries\large, primarydark] at (7,11.2) {Automatic BC Configuration by spaceTimeWindowInitCase};
    ...
    \node[smallbox, fill=white, draw=warningorange] at (0,6.8) {U.dvz[.zstd] (velocity)};
    \node[smallbox, fill=white, draw=primaryblue] at (0,6.1) {p.dvz[.zstd], gradp.dvz[.zstd]};
    \node[smallbox, fill=white, draw=gray!50] at (0,5.4) {points, metadata};
    ...
    \node[anchor=north west, font=\sffamily\tiny, text width=4.5cm, align=left] at (4.7,7.7) {
        ...
        \quad - U + data $\rightarrow$ spaceTimeWindow\\
        \quad - p + gradp $\rightarrow$ CoupledPressure\\
    ...
    \node[rectangle, rounded corners=4pt, fill=codebg, draw=codeframe, line width=1pt,
        ...
        \centering\textbf{Recommended Boundary Condition Assignment (oldInternalFaces patch)}
        \vspace{0.3em}

        \begin{tabular}{@{}l@{\;}l@{\;}l@{}}
        \textbf{Field} & \textbf{Condition} & \textbf{Assigned BC} \\[0.3em]
        \textcolor{primaryblue}{U (velocity)} & Has U in boundaryData & \texttt{spaceTimeWindow} (Dirichlet) \\
        \textcolor{secondarygreen}{p (pressure)} & Has p + gradp data & \texttt{spaceTimeWindowCoupledPressure} \\
        \textcolor{warningorange}{nut, k, $\omega$} & No boundaryData & \texttt{zeroGradient} \\
        \end{tabular}
        \end{minipage}
    };
    ...
    \draw[dashed, gray, line width=1pt, rounded corners=8pt] (-2.5,0) rectangle (16.5,10.5);
    \node[fill=white, inner sep=5pt, font=\sffamily\normalsize\bfseries, text=gray!60!black] at (7,10.5) {Laptop / Desktop Computer (Offline Reconstruction)};
\end{tikzpicture}
\caption{Recommended boundary condition configuration for pressure-coupled reconstruction. The \texttt{spaceTimeWindowInitCase} utility can assign BCs based on available data: \texttt{spaceTimeWindow} for velocity (Dirichlet), \texttt{spaceTimeWindowCoupledPressure} for pressure (mixed Dirichlet/Neumann), and \texttt{zeroGradient} for turbulence fields.}
...
\begin{itemize}
    ...
    \item \textbf{Same resolution}: Highest-fidelity reconstruction for validation
\end{itemize}
...
\begin{tikzpicture}[
    ...
    \node[rectangle, rounded corners=2pt, fill=warninglight, draw=warningorange,
          font=\sffamily\tiny, inner sep=2pt] at (2,4) {U,p,gradp stored};

    % === ARROWS TO DIFFERENT RESOLUTIONS ===

    % Arrow to coarser
    \draw[arrow, gray, line width=1.5pt] (4.5,5.5) -- node[above, font=\sffamily\footnotesize, sloped] {coarsen} (7,8.3);

    % Arrow to same
    \draw[arrow, gray, line width=1.5pt] (4.5,4) -- node[above, font=\sffamily\footnotesize] {same} (7,3.3);

    % Arrow to finer
    \draw[arrow, gray, line width=1.5pt] (4.5,2.5) -- node[below, font=\sffamily\footnotesize, sloped] {refine} (7,-2);

    % === COARSER RECONSTRUCTION ===
    \node[anchor=south, font=\sffamily\bfseries, secondarygreen] at (10,9.8) {Coarser};
    \node[font=\sffamily\tiny, gray] at (10,9.4) {\texttt{-coarsenLevel 1}};

    \fill[secondarylight, opacity=0.5] (8,7.5) rectangle (12,9);
    \draw[secondarygreen, line width=1.5pt] (8,7.5) -- (8,9);

    % Fewer target faces
    \foreach \y in {8.0, 8.6} {
        \fill[secondarygreen] (8,\y) circle (3pt);
    }
    \node[font=\sffamily\tiny, secondarygreen, anchor=west] at (8.3,8.3) {2 faces};

    % Interpolation method
    \node[rectangle, rounded corners=2pt, fill=codebg, draw=codeframe,
          font=\sffamily\tiny, inner sep=2pt, text width=2.8cm, align=center] at (10,7.2) {
    ...
    % Same number of faces (7, matching source)
    \foreach \y in {2.65, 2.9, 3.15, 3.4, 3.55, 3.75, 3.95} {
        \fill[accentpurple] (8,\y) circle (3pt);
    }
    \node[font=\sffamily\tiny, accentpurple, anchor=west] at (8.3,3.3) {7 faces};
    ...
    \node[anchor=south, font=\sffamily\bfseries, warningorange] at (10,-0.2) {Finer};
    \node[font=\sffamily\tiny, gray] at (10,-0.6) {\texttt{-refineLevel 1}};

    \fill[warninglight, opacity=0.5] (8,-3) rectangle (12,-1);
    \draw[warningorange, line width=1.5pt] (8,-3) -- (8,-1);

    % More target faces (14)
    \foreach \y in {-2.85, -2.55, -2.25, -1.95, -1.65, -1.35, -1.15} {
        \fill[warningorange] (8,\y) circle (2pt);
    }
    \foreach \y in {-2.7, -2.4, -2.1, -1.8, -1.5, -1.25, -1.05} {
        \fill[warningorange] (8,\y) circle (2pt);
    }
    \node[font=\sffamily\tiny, warningorange, anchor=west] at (8.3,-2) {14 faces};

    % Interpolation method
    \node[rectangle, rounded corners=2pt, fill=codebg, draw=codeframe,
          font=\sffamily\tiny, inner sep=2pt, text width=2.8cm, align=center] at (10,-3.3) {
    ...
    \draw[->, primaryblue, line width=1pt] (12.2,8.3) -- (12.8,4.5);
    \draw[->, primaryblue, line width=1pt] (12.2,3.3) -- (12.8,3.3);
    \draw[->, primaryblue, line width=1pt] (12.2,-2) -- (12.8,1.8);
\end{tikzpicture}
...
Encrypted files have the \texttt{.enc} extension appended to the codec extension (e.g., \texttt{U.dvz.enc}, or \texttt{U.dvz.zstd.enc} when integrated zstd compression is also enabled).
...
\begin{itemize}
    ...
    \item Encrypted boundary data (\texttt{constant/boundaryData/*.zstd.enc} or \texttt{*.enc})
...
The library provides five approaches for applying boundary conditions on the extraction boundary (\texttt{oldInternalFaces}), plus an optional ghost cell mode that can be combined with any approach:

\subsection{Pressure-Coupled BC --- Recommended}
\label{sec:pressure-coupled-bc}

The \textbf{recommended approach} for accurate reconstruction uses \texttt{spaceTime\-Window\-Coupled\-Pressure} for the pressure field, which properly handles its elliptic nature.

\begin{itemize}
    \item \textbf{Velocity (U)}: \texttt{spaceTimeWindow} --- pure Dirichlet from extracted data
    \item \textbf{Pressure (p)}: \texttt{spaceTimeWindowCoupledPressure} --- mixed Dirichlet/Neumann based on flux
    \item \textbf{Turbulence fields}: \texttt{zeroGradient}
\end{itemize}

\textbf{Why this approach is physically correct:}
\begin{itemize}
    \item Pressure is governed by an elliptic Poisson equation --- cutting the domain severs the pressure's ability to ``feel'' the flow outside the window
    \item The BC uses smooth tanh-based blending to transition between Dirichlet and Neumann limits
    \item Two blending modes are available:
    \begin{itemize}
        \item \texttt{fluxMagnitude} (default, recommended): Dirichlet at stagnant faces, Neumann at active flow faces --- more compatible with continuity
        \item \texttt{flowDirection}: Dirichlet at inflow faces, Neumann at outflow faces --- the original approach
    \end{itemize}
    \item This \textbf{reproduces the original flow} rather than computing a different solution
\end{itemize}

\begin{lstlisting}[language=OpenFOAM,caption={Pressure-coupled boundary conditions}]
// In 0/U
oldInternalFaces
{
...
oldInternalFaces
{
    ...
    transitionWidth 0.1;      // Blending sharpness
    allowTimeInterpolation  true;
    timeInterpolationScheme cubic;
    value           uniform 0;
}
...
\begin{notebox}
The pressure-coupled approach requires extracting pressure gradients during extraction. Enable this with \texttt{extractGradients true} and \texttt{gradientFields (p)} in the extraction configuration. See \cref{sec:config-extract} for details.
\end{notebox}

\subsection{Precise BC (\texttt{-preciseBC}) --- Highest Fidelity Reconstruction}
\label{sec:precise-bc}

The \texttt{-preciseBC} approach is the most "accurate" reconstruction option, using the intermediate velocity $\mathbf{u}^* = H(\mathbf{U})/A$ (commonly written \texttt{HbyA} in OpenFOAM) instead of the final velocity $\mathbf{U}$ as the boundary condition for the velocity field. 

In OpenFOAM this involves modifying the solver to expose \texttt{HbyA} to function objects.

\begin{itemize}
    \item \textbf{Velocity (U)}: \texttt{spaceTimeWindow} with \texttt{fieldTableName HbyA} --- reads extracted $\mathbf{u}^*$ data
    \item \textbf{Pressure (p)}: \texttt{spaceTimeWindowCoupledPressure} --- mixed Dirichlet/Neumann based on flux
    \item \textbf{Turbulence fields}: \texttt{zeroGradient}
\end{itemize}

\textbf{Why $\mathbf{u}^*$ is the theoretically optimal boundary condition:}

In the PIMPLE algorithm, the pressure Poisson equation is:
\[
    \nabla \cdot \left( \frac{1}{A} \nabla p \right) = \nabla \cdot \mathbf{u}^*
\]

The intermediate velocity $\mathbf{u}^*$ is the \emph{source term} of the pressure equation. If $\mathbf{u}^*$ is prescribed exactly on the boundary, the pressure equation on the subdomain becomes identical to the pressure equation on the full domain (restricted to the subdomain).

Wu et al.~\cite{Wu2020} in 2020 demonstrated machine-precision pressure recovery using this approach in a DNS context for compressing turbulence databases. Two factors contribute to this: (i) very high spatial resolution minimizes the difference between full-domain and subdomain discretization stencils at boundary cells, and (ii) DNS codes typically employ spectral or direct (non-iterative) pressure solvers that yield machine-precision solutions, unlike the iterative solvers (e.g.\ GAMG) used in finite-volume codes such as OpenFOAM. On practical LES/RANS meshes with iterative solvers, a measurable reconstruction error remains (see \cref{sec:accuracy-comparison}), even when:
\begin{itemize}
    \item Face values use OpenFOAM's exact distance-weighted interpolation formula (no spatial interpolation error), and
    \item Boundary data is available at every solver timestep (no temporal interpolation error).
\end{itemize}
The remaining error arises from the inherent difference between solving the non-linear PIMPLE equations on a full domain versus on a subdomain with prescribed boundary conditions~\cite{Jasak1996, Moukalled2016}. At boundary faces, the discretization changes from an internal-face stencil (using neighbor cell values) to a Dirichlet BC stencil. While the Laplacian gradient is algebraically identical, the non-linear convective terms, TVD/upwind flux limiters, and the multigrid pressure solver all behave differently at boundary faces~\cite{Ferziger2020, Jasak1996}, introducing small differences that compound over timesteps. It is worth noting that LES and RANS are by definition approximate solutions---the turbulence closure models introduce modelling errors that are typically orders of magnitude larger than the reconstruction error. Nevertheless, \texttt{-preciseBC} remains the most accurate option because it provides the theoretically exact source term for the pressure equation.

In contrast, using the final velocity $\mathbf{U}$ (which satisfies $\nabla \cdot \mathbf{U} = 0$) provides a divergence-free boundary condition that carries no information about the pressure source term, requiring the coupled pressure BC to reconstruct the missing information through blending.

\textbf{Requirements:}
\begin{itemize}
    \item The extraction must be performed with \texttt{pimpleFoamPrecise} (see \cref{sec:pimpleFoamPrecise}), which registers $\mathbf{u}^*$ as \texttt{HbyA} in the objectRegistry
    \item The extraction function object must use \texttt{fields (HbyA p)} with \texttt{extractGradients true} and \texttt{gradientFields (p)}
\end{itemize}

\begin{lstlisting}[language=OpenFOAM,caption={Precise boundary conditions (with \texttt{-preciseBC})}]
// In 0/U (reads HbyA data, applied to U field)
oldInternalFaces
{
    type            spaceTimeWindow;
    dataDir         "constant/boundaryData";
    fieldTableName  HbyA;       // Read HbyA data files for this field
    fixesValue      true;
    allowTimeInterpolation  true;
    timeInterpolationScheme cubic;
    value           uniform (0 0 0);
...
\begin{lstlisting}[language=bash,caption={Using precise BC}]
# Extraction with pimpleFoamPrecise
cd source-case
pimpleFoamPrecise    # Registers HbyA in objectRegistry
...
\begin{tipbox}
The \texttt{-preciseBC} approach is the highest-fidelity option available. It yields continuity errors $\sim 10^{-19}$ (machine epsilon), compared to the standard pressure-coupled approach which is accurate to solver tolerance ($\sim 10^{-6}$). The remaining reconstruction error arises from the non-linear discretization difference between internal faces (full domain) and Dirichlet boundary faces (subdomain)~\cite{Jasak1996, Moukalled2016, Ferziger2020}, not from any interpolation.
\end{tipbox}

\subsection{Accuracy Comparison}
\label{sec:accuracy-comparison}

A quantitative comparison between \texttt{-preciseBC} (using $\mathbf{u}^*$ + $\nabla p$) and the standard pressure-coupled BC (using $\mathbf{U}$ + $\nabla p$) was performed on the UFR2-02 square cylinder case (110\,720 cells full domain, 47\,952 cells subset, 88 timesteps with $\Delta t = 10^{-4}$, no time interpolation, binary uncompressed data throughout).

\begin{table}[h]
\centering
\caption{Internal field errors after 88 timesteps: subset vs.\ full domain}
\label{tab:accuracy-comparison}
\begin{tabular}{l c c c c}
\toprule
 & \multicolumn{2}{c}{\textbf{Velocity $\mathbf{U}$}} & \multicolumn{2}{c}{\textbf{Pressure $p$}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
\textbf{Method} & $L_\infty$ & $L_2$ & $L_\infty$ & $L_2$ \\
\midrule
\texttt{-preciseBC} ($\mathbf{u}^*$ + $\nabla p$) & $3.71 \times 10^{-3}$ & $3.07 \times 10^{-4}$ & $2.19 \times 10^{-3}$ & $3.22 \times 10^{-4}$ \\
Coupled pressure ($\mathbf{U}$ + $\nabla p$) & $3.65 \times 10^{-3}$ & $3.10 \times 10^{-4}$ & $2.65 \times 10^{-3}$ & $3.50 \times 10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}

Both methods start from identical binary initial conditions (zero error at $t_0$). Key observations:
\begin{itemize}
    \item \textbf{Velocity errors are nearly identical} between the two methods --- the choice of $\mathbf{u}^*$ vs.\ $\mathbf{U}$ for the velocity BC has negligible effect on velocity reconstruction.
    \item \textbf{Pressure errors are $\sim$20\% lower with \texttt{-preciseBC}}: $L_\infty = 2.19 \times 10^{-3}$ vs.\ $2.65 \times 10^{-3}$.
    \item \textbf{Errors are concentrated near the boundary.} Only 0.25\% of cells exceed 90\% of the maximum error. The top-10 worst cells are all at $x = 0.055$ (one cell layer from the extraction box face at $x = 0.05$).
\end{itemize}

\begin{table}[h]
\centering
\caption{Error decay with distance from the \texttt{oldInternalFaces} boundary (\texttt{-preciseBC})}
\label{tab:error-vs-distance}
\begin{tabular}{l r c c c c}
\toprule
\textbf{Distance from boundary} & \textbf{Cells} & \multicolumn{2}{c}{\textbf{$\mathbf{U}$ error}} & \multicolumn{2}{c}{\textbf{$p$ error}} \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6}
 & & Mean & Max & Mean & Max \\
\midrule
0.005--0.01\,(1 cell layer) & 1\,296 & $1.2 \times 10^{-3}$ & $3.7 \times 10^{-3}$ & $1.0 \times 10^{-3}$ & $2.2 \times 10^{-3}$ \\
0.01--0.02 & 5\,040 & $2.5 \times 10^{-4}$ & $9.9 \times 10^{-4}$ & $4.0 \times 10^{-4}$ & $1.5 \times 10^{-3}$ \\
0.02--0.05 & 12\,384 & $1.1 \times 10^{-4}$ & $6.5 \times 10^{-4}$ & $2.4 \times 10^{-4}$ & $8.3 \times 10^{-4}$ \\
0.05--0.10 & 15\,156 & $4.5 \times 10^{-5}$ & $2.9 \times 10^{-4}$ & $1.5 \times 10^{-4}$ & $7.5 \times 10^{-4}$ \\
0.10--0.20 & 14\,076 & $1.2 \times 10^{-5}$ & $9.2 \times 10^{-5}$ & $5.3 \times 10^{-5}$ & $5.5 \times 10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}

In this comparison, no temporal interpolation is used (boundary data is available at every solver timestep) and the face values are computed using OpenFOAM's exact distance-weighted linear interpolation formula ($\phi_f = w \, \phi_{\text{inside}} + (1-w) \, \phi_{\text{outside}}$). Despite this, a reconstruction error remains. It arises from the non-linear discretization difference between full-domain internal faces and subdomain Dirichlet boundary faces~\cite{Jasak1996, Moukalled2016}: convective flux limiters (TVD/upwind), the multigrid pressure solver (GAMG), and the PIMPLE outer correction loop all behave differently at boundary faces~\cite{Ferziger2020}, introducing small differences that compound over the 88 timesteps.

\begin{notebox}
Wu et al.~\cite{Wu2020} in 2020 report machine-precision pressure recovery in a DNS setting. Two factors explain this: the very high spatial resolution of DNS minimizes the discretization stencil mismatch at boundary cells, and DNS codes typically use spectral or direct (non-iterative) pressure solvers that produce exact solutions. In contrast, finite-volume LES/RANS codes like OpenFOAM use iterative solvers (GAMG) that converge to a tolerance, and coarser grids amplify the stencil mismatch, leading to the measured errors. Moreover, LES and RANS are inherently approximate---turbulence closure models introduce modelling errors that dwarf the reconstruction error. The advantage of $\mathbf{u}^*$ is most visible in the pressure field near the boundary ($\sim$20\% lower $L_\infty$).
\end{notebox}

\subsection{Inlet-Outlet BC (\texttt{-inletOutletBC}) --- For Quick Tests}
...
\begin{itemize}
    ...
    \item However, pressure uses \texttt{zeroGradient}, which can lead to solution drift over time
\end{itemize}
...
\begin{warningbox}
The inlet-outlet BC does not reproduce the original pressure field. The subset simulation will evolve to a \textbf{different solution} because pressure information from outside the extraction window is lost. For accurate reconstruction, use the pressure-coupled approach instead.
\end{warningbox}
...
\begin{warningbox}
The options \texttt{-preciseBC}, \texttt{-inletOutletBC}, and \texttt{-outletDirection} are mutually exclusive. Using incompatible combinations produces an error with guidance.
\end{warningbox}

\subsection{Ghost Cell Mode (\texttt{-ghostCells}) --- Machine-Precision Reconstruction}
\label{sec:ghost-cells}

The \texttt{-ghostCells} option includes one extra layer of cells (``ghost cells'') outside the extraction boundary in the sub-mesh. Ghost cell values are prescribed from the extracted global simulation data at each timestep using either an \texttt{fvOption} or explicit solver-side overwrite. This pushes the boundary stencil mismatch (see \cref{sec:concept-why}) one layer outward, where it only affects ghost cells that are overwritten anyway. The original extraction boundary faces remain \textbf{internal} in the sub-mesh, preserving the identical FVM stencil as the global simulation.

\textbf{Why ghost cells achieve machine-precision accuracy:}
\begin{itemize}
    \item Without ghost cells, extracting a subdomain converts internal faces to boundary faces, changing the FVM discretization stencil---TVD limiters, GAMG coarsening, and gradient reconstruction all behave differently at boundary faces. This limits accuracy to $\sim O(10^{-3})$.
    \item With ghost cells, the original extraction boundary faces stay internal. The stencil mismatch is pushed to the outer ghost boundary, affecting only ghost cells which are overwritten from extracted data.
    \item All ghost cell data uses direct cell-centre values (no interpolation), preserving machine-precision transfer at every stage of the pipeline.
\end{itemize}

\Cref{fig:ghost-cell-strategy} illustrates the ghost cell strategy.
...
\begin{tikzpicture}[
    ...
    scale=0.82,
    every node/.style={font=\sffamily\small},
    cellnode/.style={minimum width=1.6cm, minimum height=1.4cm, draw=gray!60, line width=0.5pt, inner sep=0pt},
]
    % ===== PANEL A: WITHOUT GHOST CELLS =====
    \node[font=\sffamily\bfseries\small, primarydark, anchor=west] at (-5.4,1.8) {(a) Without ghost cells};

    % Subdomain cells
    \node[cellnode, fill=primarylight!40] (a1) at (-3.6,0) {};
    \node[cellnode, fill=primarylight!40] (a2) at (-2.0,0) {};
    \node[cellnode, fill=primarylight!40] (a3) at (-0.4,0) {};

    % Cell center dots
    \foreach \x in {-3.6,-2.0,-0.4} {
        \filldraw[primaryblue] (\x,0) circle (2.5pt);
    }
    \node[primaryblue, font=\sffamily\tiny, anchor=north] at (-3.6,-0.45) {$\phi_i$};
    \node[primaryblue, font=\sffamily\tiny, anchor=north] at (-2.0,-0.45) {$\phi_j$};
    \node[primaryblue, font=\sffamily\tiny, anchor=north] at (-0.4,-0.45) {$\phi_k$};

    % Extraction boundary = boundary face
    \draw[warningorange, line width=3pt] (0.4,-0.7) -- (0.4,0.7);

    % Excluded domain
    \node[cellnode, fill=gray!15, draw=gray!30] (ex1) at (1.6,0) {};
    \node[cellnode, fill=gray!15, draw=gray!30] (ex2) at (3.2,0) {};
    \filldraw[gray!40] (1.6,0) circle (2pt);
    \filldraw[gray!40] (3.2,0) circle (2pt);
    \node[gray!50, font=\sffamily\tiny] at (2.4,-1.0) {Excluded};

    % Boundary face label
    \node[warningorange, font=\sffamily\footnotesize, anchor=south, text width=2.2cm, align=center] at (0.4,0.85) {Patch\\boundary};

    % Problem annotation: cell k stencil changed
    \draw[red!70, line width=1.2pt, dashed, rounded corners=2pt] (-1.15,-0.62) rectangle (0.35,0.62);

    % Arrow pointing to the problem
    \draw[->, red!70, line width=1pt] (1.7,1.5) -- (0.1,0.7);
    \node[red!70, font=\sffamily\footnotesize, anchor=west, text width=3.8cm] at (1.8,1.7) {Cell $k$: stencil changed\\(boundary face, not internal)};

    % Subdomain label
    \node[primarydark, font=\sffamily\footnotesize] at (-2.0,-1.0) {Subdomain};

    % ===== PANEL B: WITH GHOST CELLS =====
    \node[font=\sffamily\bfseries\small, primarydark, anchor=west] at (-5.4,-2.3) {(b) With ghost cells};

    % Subdomain cells
    \node[cellnode, fill=primarylight!40] (b1) at (-3.6,-4.1) {};
    \node[cellnode, fill=primarylight!40] (b2) at (-2.0,-4.1) {};
    \node[cellnode, fill=primarylight!40] (b3) at (-0.4,-4.1) {};

    % Ghost cell
    \node[cellnode, fill=warninglight, draw=warningorange!60] (g1) at (1.2,-4.1) {};
    \node[warningorange, font=\sffamily\footnotesize\bfseries] at (1.2,-4.1) {Ghost};

    % Cell center dots
    \foreach \x in {-3.6,-2.0,-0.4} {
        \filldraw[primaryblue] (\x,-4.1) circle (2.5pt);
    }
    \filldraw[warningorange] (1.2,-4.1) circle (2.5pt);

    \node[primaryblue, font=\sffamily\tiny, anchor=north] at (-3.6,-4.55) {$\phi_i$};
    \node[primaryblue, font=\sffamily\tiny, anchor=north] at (-2.0,-4.55) {$\phi_j$};
    \node[primaryblue, font=\sffamily\tiny, anchor=north] at (-0.4,-4.55) {$\phi_k$};

    % Original extraction boundary = INTERNAL face
    \draw[secondarygreen, line width=2.5pt] (0.4,-4.8) -- (0.4,-3.4);
    \node[secondarygreen, font=\sffamily\footnotesize, anchor=south, text width=2.2cm, align=center] at (0.4,-3.2) {Internal\\face~$\checkmark$};

    % Outer boundary
    \draw[gray!50, line width=2pt] (2.0,-4.8) -- (2.0,-3.4);
    \node[gray!50, font=\sffamily\footnotesize, anchor=south] at (2.0,-3.2) {\texttt{zeroGrad}};

    % Excluded domain
    \node[cellnode, fill=gray!15, draw=gray!30, minimum width=1.4cm] (ex3) at (3.0,-4.1) {};
    \filldraw[gray!40] (3.0,-4.1) circle (2pt);
    \node[gray!50, font=\sffamily\tiny] at (3.0,-5.1) {Excluded};

    % Arrow showing ghost cell overwrite
    \draw[->, warningorange, line width=1.2pt] (1.2,-2.5) -- (1.2,-3.3);
    \node[warningorange, font=\sffamily\footnotesize, anchor=south, text width=2.8cm, align=center] at (1.2,-2.5) {Overwritten from\\extracted data};

    % Happy annotation: cell k stencil preserved
    \draw[secondarygreen, line width=1.2pt, dashed, rounded corners=2pt] (-1.15,-4.72) rectangle (0.35,-3.48);
    \draw[->, secondarygreen, line width=1pt] (-2.3,-2.7) -- (-0.6,-3.4);
    \node[secondarygreen, font=\sffamily\footnotesize, anchor=east, text width=3.6cm, align=right] at (-2.4,-2.5) {Cell $k$: stencil preserved\\(internal face with neighbor)};

    % Subdomain label
    \node[primarydark, font=\sffamily\footnotesize] at (-2.0,-5.1) {Subdomain};

\end{tikzpicture}
\caption{Ghost cell strategy. (a)~Without ghost cells, the extraction boundary becomes a patch boundary face, altering the FVM discretization stencil for adjacent cells (different limiters, gradient reconstruction, and GAMG coarsening behaviour). (b)~With one layer of ghost cells, the original extraction boundary remains an internal face with the identical stencil as in the global simulation. Ghost cell values are overwritten from extracted data at each timestep; the outer boundary uses \texttt{zeroGradient} (its effect is confined to ghost cells that are overwritten anyway).}
\label{fig:ghost-cell-strategy}
\end{figure}

\textbf{Two ghost cell forcing mechanisms are provided:}

\begin{enumerate}
    \item \textbf{ghostCellConstraint fvOption} (recommended): Overrides the matrix equation in ghost cells via \texttt{eqn.setValues()}. Works with any solver that supports \texttt{fvOptions.constrain()}.
    \item \textbf{Explicit overwrite} (built into \texttt{pimpleFoamStag} and \texttt{pimpleFoamPrecise}): Directly writes ghost cell values into the field arrays. Activated automatically when a \texttt{ghostCells} cellZone exists in the mesh.
\end{enumerate}

\begin{lstlisting}[language=OpenFOAM,caption={Extraction configuration with ghost cells}]
extractSubset
{
    type            spaceTimeWindowExtract;
    libs            (spaceTimeWindow);
...
\begin{lstlisting}[language=bash,caption={Ghost cell reconstruction workflow}]
...
\begin{lstlisting}[language=OpenFOAM,caption={ghostCellConstraint fvOption configuration}]
...
\begin{notebox}
When \texttt{-ghostCells} is used, the \texttt{oldInternalFaces} patch moves one layer outward (to the ghost cell outer boundary). All fields on this patch receive \texttt{zeroGradient} BCs by default, since the ghost cells are overwritten from extracted data and the outer boundary condition does not affect the interior solution.
\end{notebox}

\begin{notebox}
Ghost cell mode can be combined with any BC approach. For example, \texttt{-ghostCells} with \texttt{-preciseBC} uses ghost cell forcing for the interior stencil and u$^*$ data on the (now-internal) original extraction boundary.
\end{notebox}
...
\toprule
\textbf{Mode} & \textbf{Field} & \textbf{In BD?} & \textbf{Type} & \textbf{BC Applied} \\
\midrule
\texttt{-preciseBC} & U & --- & vector & \texttt{spaceTimeWindow} + \texttt{fieldTableName HbyA} \\
\texttt{-preciseBC} & p & --- & scalar & \texttt{spaceTimeWindowCoupledPressure} \\
\texttt{-preciseBC} & other & No & any & \texttt{zeroGradient} \\
\midrule
\texttt{-inletOutletBC} & --- & Yes & vector & \texttt{spaceTimeWindowInletOutlet} \\
\texttt{-inletOutletBC} & --- & Yes & scalar & \texttt{zeroGradient} \\
\midrule
default & --- & Yes & any & \texttt{spaceTimeWindow} (Dirichlet) \\
any & --- & No & any & \texttt{zeroGradient} \\
\bottomrule
\end{tabularx}
\end{table}

This ensures fields without time-varying data automatically get appropriate BCs. With \texttt{-preciseBC}, the U and p fields are handled specially regardless of boundaryData contents (U reads HbyA data via \texttt{fieldTableName}).
...
functions
{
    extractSubset
    {
        ...
        // Gradient extraction for pressure-coupled BC (RECOMMENDED)
        extractGradients    true;
        gradientFields      (p);          // Extract normal gradient of p
        ...
        // Ghost cell extraction (for machine-precision reconstruction)
        // extractGhostCells true;
...
\texttt{extractGradients} & bool & No & Enable gradient extraction (default: false) \\
\texttt{gradientFields} & wordList & No & Fields for gradient extraction (requires extractGradients) \\
\texttt{extractGhostCells} & bool & No & Extract ghost cell data for machine-precision reconstruction (default: false) \\
...
For the highest-fidelity approach with \texttt{-preciseBC} (requires \texttt{pimpleFoamPrecise}):

\begin{lstlisting}[language=OpenFOAM,caption={Field selection for precise BC (highest fidelity)}]
// In extraction function object (run with pimpleFoamPrecise)
fields          (HbyA p);         // HbyA (intermediate velocity) and p for time-varying BC
initialFields   (U p nut);        // U (final velocity) needed for solver initialization
extractGradients    true;         // Enable gradient extraction
gradientFields      (p);          // Extract normal gradient of p (creates gradp)
\end{lstlisting}

For the standard pressure-coupled approach:

\begin{lstlisting}[language=OpenFOAM,caption={Field selection strategy for pressure-coupled BC}]
// In extraction function object (run with pimpleFoam)
fields          (U p);             // U and p for time-varying BC
initialFields   (U p nut);         // nut needed for turbulence model
extractGradients    true;          // Enable gradient extraction
gradientFields      (p);           // Extract normal gradient of p (creates gradp)
\end{lstlisting}

Note: \texttt{nut} is only needed in \texttt{initialFields} (not \texttt{fields}) since the turbulence model computes it at runtime and uses \texttt{zeroGradient} BC on \texttt{oldInternalFaces}.

Fields NOT in \texttt{boundaryData} automatically get \texttt{zeroGradient} BC on \texttt{oldInternalFaces}.

\begin{tipbox}
For accurate reconstruction, use the \textbf{pressure-coupled BC approach}: extract both \texttt{U} and \texttt{p} with gradient extraction enabled for \texttt{p}. This allows the \texttt{spaceTimeWindowCoupledPressure} BC to properly constrain the pressure field and reproduce the original solution.
\end{tipbox}

\begin{notebox}
When \texttt{extractGradients} is enabled, the extractor computes the face-normal gradient at each boundary face using the equation:
\[
\left.\frac{\partial \phi}{\partial n}\right|_f = \frac{\phi_N - \phi_P}{|\mathbf{d}_{PN}|}
\]
where $\phi_P$ is the value at the face owner cell, $\phi_N$ is the value at the neighbour cell, and $\mathbf{d}_{PN}$ is the vector from owner to neighbour cell centre. The gradient data is written to files named \texttt{grad<fieldName>} (e.g., \texttt{gradp}) in the boundaryData directory.
\end{notebox}

\subsection{Boundary Condition Configuration}
\label{sec:config-bc}

\subsubsection{spaceTimeWindowCoupledPressure (Recommended for Pressure)}
\label{sec:bc-coupled-pressure}

Mixed (Robin) boundary condition for pressure that blends Dirichlet and Neumann conditions based on local flux. This is the \textbf{recommended approach} for pressure at extraction boundaries. Two blending modes are available:

\begin{itemize}
    \item \texttt{fluxMagnitude} (default, recommended): Dirichlet at stagnant faces (zero flux), Neumann at active flow faces --- better continuity compatibility
    \item \texttt{flowDirection}: Dirichlet at inflow, Neumann at outflow --- the original flow-direction-based approach
\end{itemize}

The parameters are listed in \cref{tab:bc-coupled-pressure-params}.

\begin{lstlisting}[language=OpenFOAM,caption={spaceTimeWindowCoupledPressure configuration}]
boundaryField
{
    oldInternalFaces
    {
...
\textbf{How it works:}
\begin{enumerate}
    \item Reads pressure values and pressure gradients from boundaryData
    \item Computes local flux at each face
    \item Calculates blending weight using smooth tanh transition based on the selected mode:
    \begin{itemize}
        \item \texttt{fluxMagnitude}:
        \[ w = w_D + (w_N' - w_D) \cdot \tanh\!\bigl(|\phi_f| / \phi_{\max} / \sigma\bigr) \]
        where $w_D$ is \texttt{dirichletWeight} and $w_N' = 1 - w_N$
        \item \texttt{flowDirection}:
        \[ w = \tfrac{1}{2}\bigl[(w_D + w_N') - (w_D - w_N') \cdot \tanh\!\bigl(\phi_f / \phi_{\max} / \sigma\bigr)\bigr] \]
    \end{itemize}
    \item In \texttt{fluxMagnitude} mode: at \textbf{stagnant faces} ($w \to w_D$) applies Dirichlet; at \textbf{high-flux faces} ($w \to w_N'$) applies Neumann
    \item In \texttt{flowDirection} mode: at \textbf{inflow faces} ($w \to w_D$) applies Dirichlet; at \textbf{outflow faces} ($w \to w_N'$) applies Neumann
\end{enumerate}

The value coefficients are:
\begin{align}
    \text{valueFraction} &= w \cdot w_D + (1 - w) \cdot 0 = w \cdot w_D \\
    \text{refValue} &= p_{\text{extracted}} \\
    \text{refGrad} &= \left.\frac{\partial p}{\partial n}\right|_{\text{extracted}}
\end{align}
where $w_D$ is \texttt{dirichletWeight} and the gradient coefficient is $(1-w) \cdot w_N$ with $w_N$ being \texttt{neumannWeight}.

\begin{table}[htbp]
\centering
\caption{spaceTimeWindowCoupledPressure parameters}
\label{tab:bc-coupled-pressure-params}
\begin{tabularx}{\textwidth}{@{}lllX@{}}
\toprule
...
\texttt{blendingMode} & word & \texttt{fluxMagnitude} & \texttt{fluxMagnitude} or \texttt{flowDirection} \\
\texttt{dirichletWeight} & scalar & 0.8 & Dirichlet contribution strength (0-1) \\
\texttt{neumannWeight} & scalar & 0.8 & Neumann contribution strength (0-1) \\
\texttt{transitionWidth} & scalar & 0.1 & Sharpness of flux transition \\
...
\begin{notebox}
This BC requires gradient data in boundaryData. Enable gradient extraction with \texttt{extractGradients true} and \texttt{gradientFields (p)} in the extraction configuration. The gradient file must be named \texttt{gradp} in each timestep directory.
\end{notebox}

\subsubsection{spaceTimeWindowInletOutlet}
...
\texttt{-preciseBC} & flag & Use HbyA-based velocity BC and coupledPressure for p (requires \texttt{pimpleFoamPrecise} extraction) \\
\texttt{-inletOutletBC} & flag & Use flux-based inlet-outlet BC for U, zeroGradient for scalars (for quick tests) \\
...
\texttt{-ghostCells} & flag & Include one layer of ghost cells outside extraction boundary for machine-precision reconstruction (see \cref{sec:ghost-cells}) \\
...
\begin{warningbox}
\texttt{-preciseBC}, \texttt{-inletOutletBC}, and \texttt{-outletDirection} are mutually exclusive. Using incompatible combinations produces an error with guidance on which option to choose.
\end{warningbox}
    ...
    constant/
        ...
        boundaryData/
            oldInternalFaces/
                ...
                0.0001/
                    U               # or U.dvz, U.dvz.zstd, U.dvz.zstd.enc
                    p               # pressure (for coupled BC)
                    gradp           # pressure gradient (for coupled BC)
                0.0002/
                    ...
            ghostCells/             # Only when extractGhostCells is true
                cellCentres         # Ghost cell positions (written once)
                ghostCellMap        # Data index to subset cell index mapping
                0.0001/
                    U               # Cell-centre values per timestep
                    p
                0.0002/
                    ...
...
deltaVarint + zstd & .dvz.zstd & $\sim$0.4\% & Integrated zstd (auto if libzstd) \\
dvzt & .dvzt & $\sim$2.4\% & Best compression, recommended \\
dvzt + zstd & .dvzt.zstd & $\sim$0.3\% & Best compression + integrated zstd \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.9]
    ...
    \node[font=\tiny\sffamily, color=bodytext] at (1.75,4.2) {writes .dvzt[.zstd] files};
    ...
    \node[font=\small\sffamily\bfseries, color=accentpurple] at (6.25,4.65) {.dvzt[.zstd]};
    \node[font=\tiny\sffamily, color=bodytext] at (6.25,4.25) {compact storage};

    % Arrow
    \draw[-{Stealth[length=3mm]}, thick, warningorange] (7.7,4.5) -- (8.3,4.5);

    % InitCase
    \draw[secondarygreen, thick, fill=secondarylight, rounded corners=4pt] (8.5,4) rectangle (13,5);
    \node[font=\scriptsize\sffamily\bfseries, color=secondarygreen] at (10.75,4.65) {spaceTimeWindowInitCase};
    \node[font=\tiny\sffamily, color=bodytext] at (10.75,4.25) {converts to .dvz[.zstd]};

    % Arrow down
    \draw[-{Stealth[length=3mm]}, thick, warningorange] (10.75,3.8) -- (10.75,3.2);

    % DVZ files
    \draw[primaryblue, thick, fill=primarylight, rounded corners=4pt] (8.5,2.2) rectangle (13,3);
    \node[font=\small\sffamily\bfseries, color=primarydark] at (10.75,2.85) {.dvz[.zstd]};
    \node[font=\tiny\sffamily, color=bodytext] at (10.75,2.45) {self-contained timesteps};

    % Arrow
    \draw[-{Stealth[length=3mm]}, thick, warningorange] (8.3,2.6) -- (7.7,2.6);
    ...
    % Notes
    \node[font=\scriptsize\sffamily, color=bodytext, align=center] at (6.25,1.5) {Delta frames require sequential decoding\\$\Rightarrow$ converted to DVZ[.zstd] before runtime};
\end{tikzpicture}
\caption{DVZT workflow: extraction writes compact .dvzt[.zstd] files. Before reconstruction, spaceTimeWindowInitCase converts them to .dvz[.zstd] format (required for random timestep access at runtime). The zstd compression layer is preserved through the conversion}
...
During extraction, DVZT writes smaller \texttt{.dvzt} files (or \texttt{.dvzt.zstd} with integrated zstd). During case initialization, the utility automatically converts DVZT to DVZ format (required because delta frames need sequential processing). The zstd layer is preserved through the conversion. The resulting \texttt{.dvz} (or \texttt{.dvz.zstd}) files are read by the spaceTimeWindow BC at runtime.

\begin{center}
Extraction $\rightarrow$ \texttt{.dvzt[.zstd]} files $\rightarrow$ \texttt{spaceTimeWindowInitCase} $\rightarrow$ \texttt{.dvz[.zstd]} files $\rightarrow$ Runtime
\end{center}
...
\subsubsection{Integrated Zstd Compression}
\label{sec:zstd}

When built with libzstd support (auto-detected by \texttt{./Allwmake}), DVZ and DVZT data is automatically wrapped with zstd level-3 compression before writing to disk. This is applied transparently as a compression layer in the encoding pipeline:

\begin{center}
codec encode $\rightarrow$ zstd compress $\rightarrow$ [optional encrypt] $\rightarrow$ write to disk
\end{center}

\textbf{Extension stacking:}
\begin{itemize}
    \item \texttt{fieldName.dvz.zstd} --- DVZ with integrated zstd
    \item \texttt{fieldName.dvzt.zstd} --- DVZT with integrated zstd
    \item \texttt{fieldName.dvz.zstd.enc} --- DVZ + zstd + encryption
    \item \texttt{fieldName.dvzt.zstd.enc} --- DVZT + zstd + encryption
\end{itemize}

The zstd layer provides an additional $\sim$10$\times$ reduction on top of the already-compact varint encoding, bringing typical file sizes to $\sim$0.3--0.4\% of uncompressed ASCII. Decompression is transparent at runtime---the boundary conditions automatically detect and decompress \texttt{.zstd} files.

\begin{notebox}
No configuration is needed: zstd compression is applied automatically when the library is built with libzstd. Files without the \texttt{.zstd} extension (from older builds or builds without libzstd) remain fully readable for backward compatibility.
\end{notebox}

The DVZT workflow with integrated zstd becomes:

\begin{center}
\texttt{.dvzt.zstd} files $\rightarrow$ \texttt{spaceTimeWindowInitCase} $\rightarrow$ \texttt{.dvz.zstd} files $\rightarrow$ Runtime
\end{center}

\subsubsection{External Compression Benchmark}

\begin{notebox}
With integrated zstd compression (built with libzstd), boundary data is already compressed on the fly with zstd -3. The benchmarks below show what is achievable with \emph{external} tools on \emph{uncompressed} DVZ/DVZT files. For archival of already zstd-compressed files, use 7z LZMA2 for additional gains.
\end{notebox}
...
\begin{itemize}
    \item \textbf{Runtime/on-the-fly}: Use integrated zstd (build with libzstd, automatic)
...
7z a -m0=lzma2 -mx=9 ../boundaryData.7z */*.dvz.zstd */*.dvz */*.dvzt.zstd */*.dvzt

# Or with tar (files already zstd-compressed if built with libzstd)
...
\begin{lstlisting}[language=OpenFOAM,caption={extractionMetadata contents}]
{
    ...
    nGlobalFaces        12345;
    extractGhostCells   true;       // present when ghost cells enabled
    nGhostCells         432;        // number of ghost cells
...
\subsubsection{Use Pressure-Coupled BC (Recommended)}

The \texttt{spaceTimeWindowCoupledPressure} BC handles mass conservation while accurately reproducing the original pressure field:
\begin{itemize}
    \item Uses mixed Dirichlet/Neumann based on local flux
    \item Two modes: \texttt{fluxMagnitude} (default, Dirichlet at stagnant faces) or \texttt{flowDirection} (Dirichlet at inflow)
    \item Neumann faces allow natural pressure adjustment for continuity compatibility
    \item Requires gradient extraction during source simulation
\end{itemize}

See \cref{sec:pressure-coupled-bc} for detailed configuration.

\subsubsection{Use \texttt{-inletOutletBC} (for quick tests)}
...
\midrule
\texttt{-ghostCells} &
\textbf{Machine-precision}. Eliminates boundary stencil mismatch by including ghost cells. Requires \texttt{pimpleFoamStag} or \texttt{pimpleFoamPrecise}. \\
\addlinespace
\texttt{-preciseBC} &
\textbf{Highest fidelity}. Most accurate pressure recovery using HbyA ($\mathbf{u}^*$). Requires \texttt{pimpleFoamPrecise} extraction. \\
\addlinespace
\texttt{-ghostCells} + \texttt{-preciseBC} &
\textbf{Maximum fidelity}. Ghost cells + $\mathbf{u}^*$ boundary data. \\
\addlinespace
\texttt{-inletOutletBC} &
Recommended for quick tests. Natural mass balance through flux-based switching. \\
\addlinespace
...
All faces prescribed with exact mass balance. \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
\texttt{none} (exact) & $t_0$ & 1 (exact match) & Highest fidelity (no interpolation error) \\
...
\begin{itemize}
    ...
    \item Ensures highest fidelity (no time interpolation error)
\end{itemize}
...
FOAM FATAL ERROR:
Failed to decrypt file: constant/boundaryData/.../U.dvz.zstd.enc
\end{lstlisting}
...
\section{Acknowledging this work}
\label{sec:acknowledging}

If this software has been useful in your research, please consider citing:

\begin{quote}
Anton, A.-A. (2011). \emph{``Space-Time Window Reconstruction in Parallel High Performance Numeric Simulations. Application for CFD''}, PhD Thesis, Politehnica University of Timisoara. Available at: \url{https://dspace.upt.ro/jspui/handle/123456789/643}
\end{quote}

\begin{lstlisting}[language=bash,caption={BibTeX entry}]
@phdthesis{Anton2011,
  author    = "Alin-Adrian Anton",
  title     = "Space-Time Window Reconstruction in High-Performance
               Numeric Simulations: Application for {CFD}",
...
Or acknowledge: ``This work made use of the openfoam-spaceTimeWindow library available at \url{https://github.com/cherubimro/openfoam-spaceTimeWindow} / .''

Secondary link: \url{https://dev.cs.upt.ro/alin.anton/openfoam-spaceTimeWindow}.
...
This library implements the space-time window reconstruction concept~\cite{Anton2011, Anton2011b}.

The example case uses mesh generation from the ERCOFTAC Classic Collection Database~\cite{ERCOFTAC043}.

The square cylinder benchmark setup was published by Niklas Nordin~\cite{Niklas2010}.
...
\subsubsection{CFD Validation Philosophy}

In computational fluid dynamics, validation means demonstrating that a simulation reproduces the \emph{physics} of the flow---not that it produces bit-identical numbers. A validated CFD result agrees with experimental measurements (velocity profiles, pressure distributions, turbulent statistics, shedding frequencies, etc.) within quantified uncertainties. Different meshes, turbulence models, numerical schemes, and even different software packages will produce numerically different fields, yet all of them can be equally ``valid'' if they capture the physical phenomena to the required accuracy.

This distinction is central to the space-time window approach. The reconstruction does not aim to be a bit-exact copy of the original simulation; it aims to be a \emph{physically faithful} re-simulation of the same flow inside the region of interest. The governing equations, boundary conditions, and fluid properties are identical; what changes is the domain extent and, optionally, the mesh resolution. The reconstructed fields are therefore validated in the same way as any other CFD result: by comparing physical quantities against reference data (experiment or a trusted full-domain simulation).

A concrete example of this validation philosophy in OpenFOAM is the Timi\c{s}oara Swirl Generator~\cite{Petit2011}, a turbomachinery test rig developed at Politehnica University of Timi\c{s}oara featuring guide vanes, a free runner with 10 blades, and a convergent-divergent draft tube. Petit et al.\ performed unsteady simulations using OpenFOAM's \texttt{transientSimpleDyMFoam} solver on a block-structured hexahedral mesh with GGI interfaces, and validated the results against Laser Doppler Velocimetry (LDV) measurements and unsteady pressure recordings at multiple stations in the draft tube~\cite{Petit2011}. The validation criteria were physical: mean and fluctuating velocity profiles, pressure recovery, and vortex rope frequency---not numerical identity with any previous computation. This case, documented on the OpenFOAM Wiki\footnote{\url{https://openfoamwiki.net/index.php/Sig_Turbomachinery_/_Timisoara_Swirl_Generator}}, illustrates the general principle that CFD validation is always performed against the physics of the flow.

\subsubsection{Validating the Reconstruction}
...
Because the reconstruction solves the same equations with the same physical parameters, any discrepancy with the full-domain solution is due to the finite-volume discretisation change at the extraction boundary (see \cref{sec:concept-why}), not to different physics. These errors are localised near the boundary and decay rapidly with distance, as demonstrated by the accuracy comparison in \cref{sec:accuracy-comparison}.

\begin{notebox}
The reconstruction should closely reproduce the original flow within the extraction region (to solver tolerance) when using exact timestep matching. Small differences may occur at the outlet boundary due to the different boundary condition type. When the mesh resolution is changed (refinement or coarsening), additional spatial interpolation errors appear, but the physical behaviour---vortex dynamics, mean profiles, spectral content---is preserved.
\end{notebox}
...
\section{The \texttt{pimpleFoamPrecise} Solver}
\label{sec:pimpleFoamPrecise}

The \texttt{pimpleFoamPrecise} solver is a modified version of OpenFOAM's standard \texttt{pimpleFoam} that registers the intermediate velocity $\mathbf{u}^* = H(\mathbf{U})/A$ (named \texttt{HbyA}) in the objectRegistry, making it accessible to function objects such as \texttt{spaceTimeWindowExtract}.

\subsection{Motivation}

In the PIMPLE algorithm, the momentum equation is semi-discretized as:
\[
    A \mathbf{U} = H(\mathbf{U}) - \nabla p
\]

The intermediate velocity $\mathbf{u}^* = H(\mathbf{U})/A$ is computed before the pressure correction step. The pressure Poisson equation is then:
\[
    \nabla \cdot \left( \frac{1}{A} \nabla p \right) = \nabla \cdot \mathbf{u}^*
\]

Wu, Zaki, and Meneveau~\cite{Wu2020} in 2020 showed that using $\mathbf{u}^*$ instead of the final divergence-free velocity $\mathbf{U}$ as the boundary condition for subdomain reconstruction yields the highest fidelity pressure recovery. In their DNS context (very fine grids), the high resolution minimizes the non-linear discretization stencil mismatch between full-domain internal faces and subdomain boundary faces~\cite{Jasak1996, Moukalled2016}, and the use of spectral/direct (non-iterative) pressure solvers yields exact solutions, together achieving machine-precision accuracy. On coarser LES/RANS meshes with iterative solvers (e.g.\ GAMG), the mismatch is larger, though it remains negligible compared to the inherent modelling error of turbulence closures. $\mathbf{u}^*$ remains the theoretically optimal choice: it is the source term of the pressure equation, so prescribing it on the boundary makes the subdomain pressure equation consistent with the full-domain one. The boundary data uses OpenFOAM's exact distance-weighted interpolation formula, and when extraction is performed at every solver timestep no temporal interpolation is needed.

\subsection{Implementation}

The solver differs from standard \texttt{pimpleFoam} in two files:

\begin{itemize}
    \item \textbf{\texttt{createFields.H}}: Declares a persistent \texttt{volVectorField} named \texttt{HbyA\_stored} with \texttt{IOobject::REGISTER} that survives across the time loop.  This is necessary because the local \texttt{HbyA} variable in \texttt{pEqn.H} is stack-allocated and its destructor unregisters it from the objectRegistry before function objects execute.

    \item \textbf{\texttt{pEqn.H}}: Copies the computed \texttt{HbyA} to the persistent field via \texttt{HbyA\_stored~=~HbyA} at every PIMPLE pressure correction iteration.
\end{itemize}

\begin{lstlisting}[language=C++,caption={Persistent HbyA field in createFields.H}]
// Persistent HbyA field for function object access
...
\begin{warningbox}
The \texttt{IOobject::REGISTER} flag is essential in OpenFOAM v2512. Without it, the field is not found by \texttt{mesh.findObject<volVectorField>("HbyA")} in the extractor function object.
\end{warningbox}

\subsection{Usage}

Use \texttt{pimpleFoamPrecise} as a drop-in replacement for \texttt{pimpleFoam} during the extraction phase:

\begin{lstlisting}[language=bash,caption={Using pimpleFoamPrecise for extraction}]
# In source case controlDict: application pimpleFoamPrecise;
# Extraction function object uses: fields (HbyA p);
cd source-case
pimpleFoamPrecise
\end{lstlisting}

The reconstruction phase uses standard \texttt{pimpleFoam} --- no special solver is needed for reconstruction.

\begin{notebox}
\texttt{pimpleFoamPrecise} adds negligible overhead compared to standard \texttt{pimpleFoam}. The only additional cost is a single field copy (\texttt{HbyA\_stored = HbyA}) per pressure correction iteration.
\end{notebox}
...
\section{The \texttt{pimpleFoamStag} Solver}
\label{sec:pimpleFoamStag}

The \texttt{pimpleFoamStag} solver implements a Mahesh-style~\cite{Mahesh2004} hybrid staggered mesh approach for incompressible, turbulent flow. Face-normal velocity \texttt{Un} is the primary kinematic variable, eliminating Rhie-Chow interpolation~\cite{Rhie1983} and providing exact discrete mass conservation.

\subsection{Algorithm}

The algorithm per PIMPLE iteration is:

\begin{enumerate}
    \item \textbf{Reconstruct}: Cell-centred $\mathbf{U}$ from face-normal $U_n$ via least-squares
    \item \textbf{Momentum}: Cell-centred UEqn using standard \texttt{fvm}/\texttt{fvc} operators
    \item \textbf{Project}: HbyA to face-normal $U_n^*$
    \item \textbf{Pressure}: $\nabla \cdot (r_{AU_f} \nabla p) = \nabla \cdot \phi^*$
    \item \textbf{Correct}: $U_n = U_n^* - r_{AU_f} \, \partial p / \partial n$ (exact, no Rhie-Chow)
    \item \textbf{Turbulence}: \texttt{turbulence->correct()} on final iteration
\end{enumerate}

\subsection{Colocated vs Staggered Discretization}
\label{sec:colocated-vs-staggered}

The fundamental difference between \texttt{pimpleFoamPrecise} (and standard \texttt{pimpleFoam}) and \texttt{pimpleFoamStag} lies in where the velocity unknowns are stored, as illustrated in \cref{fig:colocated-vs-staggered}.
...
\begin{tikzpicture}[
    ...
    % ===== LEFT: COLOCATED ARRANGEMENT =====
    \node[font=\sffamily\bfseries\small, primarydark] at (2.25,5.0) {(a) Colocated};
    \node[font=\sffamily\footnotesize, bodytext] at (2.25,4.6) {(\texttt{pimpleFoam}, \texttt{pimpleFoamPrecise})};

    % Grid lines
    \draw[gray!40, line width=0.5pt] (0,0) grid[step=1.5] (4.5,4.5);
    \draw[gray!70, line width=1pt] (0,0) rectangle (4.5,4.5);

    % Cell center dots (U, p) - show labels only in center row for clarity
    \foreach \i in {0.75,2.25,3.75} {
        \foreach \j in {0.75,2.25,3.75} {
            \filldraw[primaryblue] (\i,\j) circle (3pt);
        }
    }
    % Labels for center column
    \node[primaryblue, font=\sffamily\tiny, anchor=south east] at (0.65,0.85) {$\mathbf{U},p$};
    \node[primaryblue, font=\sffamily\tiny, anchor=south east] at (0.65,2.35) {$\mathbf{U},p$};
    \node[primaryblue, font=\sffamily\tiny, anchor=south east] at (0.65,3.85) {$\mathbf{U},p$};

    % Rhie-Chow interpolation illustration (between two cells)
    \draw[->, red!60, line width=1.3pt, shorten >=3pt] (0.75,2.25) -- (1.5,2.25);
    \draw[->, red!60, line width=1.3pt, shorten >=3pt] (2.25,2.25) -- (1.5,2.25);
    \filldraw[warningorange] (1.5,2.25) circle (2.5pt);

    % Rhie-Chow label with brace
    \draw[decorate, decoration={brace, amplitude=4pt, mirror}, warningorange, line width=0.8pt]
        (0.85,1.85) -- (2.15,1.85);
    \node[warningorange, font=\sffamily\tiny, anchor=north, text width=2.2cm, align=center] at (1.5,1.7) {Rhie-Chow\\interpolation};

    % Checkerboard risk annotation
    \node[red!60, font=\sffamily\tiny, anchor=north west, text width=2.5cm] at (3.0,1.2) {Risk: pressure\\checkerboard};

    % Legend
    \filldraw[primaryblue] (0.0,-0.6) circle (3pt);
    \node[anchor=west, font=\sffamily\footnotesize] at (0.2,-0.6) {$\mathbf{U}, p$ at cell centre};
    \filldraw[warningorange] (0.0,-1.1) circle (2.5pt);
    \node[anchor=west, font=\sffamily\footnotesize] at (0.2,-1.1) {Face flux (interpolated)};

    % ===== RIGHT: STAGGERED ARRANGEMENT =====
    \node[font=\sffamily\bfseries\small, primarydark] at (8.75,5.0) {(b) Hybrid staggered~\cite{Mahesh2004}};
    \node[font=\sffamily\footnotesize, bodytext] at (8.75,4.6) {(\texttt{pimpleFoamStag})};

    % Grid lines
    \draw[gray!40, line width=0.5pt] (6.5,0) grid[step=1.5] (11.0,4.5);
    \draw[gray!70, line width=1pt] (6.5,0) rectangle (11.0,4.5);

    % Cell center pressure (p only)
    \foreach \i in {7.25,8.75,10.25} {
        \foreach \j in {0.75,2.25,3.75} {
            \filldraw[accentpurple] (\i,\j) circle (3pt);
        }
    }
    % Labels for first column
    \node[accentpurple, font=\sffamily\tiny, anchor=south east] at (7.15,0.85) {$p$};
    \node[accentpurple, font=\sffamily\tiny, anchor=south east] at (7.15,2.35) {$p$};
    \node[accentpurple, font=\sffamily\tiny, anchor=south east] at (7.15,3.85) {$p$};

    % Face-normal velocity Un at vertical faces (small horizontal arrows)
    \foreach \x in {8.0,9.5} {
        \foreach \j in {0.75,2.25,3.75} {
            \draw[->, secondarygreen, line width=1.6pt] ({\x-0.22},\j) -- ({\x+0.22},\j);
        }
    }

    % Face-normal velocity Un at horizontal faces (small vertical arrows)
    \foreach \i in {7.25,8.75,10.25} {
        \foreach \y in {1.5,3.0} {
            \draw[->, secondarygreen, line width=1.6pt] (\i,{\y-0.22}) -- (\i,{\y+0.22});
        }
    }

    % Direct flux annotation
    \node[secondarygreen, font=\sffamily\tiny, anchor=west, text width=1.8cm] at (9.7,1.5) {$U_n$: exact\\face flux};

    % Reconstruction annotation (Un -> U)
    \draw[->, gray, line width=0.8pt, dashed] (8.0,3.75) -- (8.75,3.75);
    \node[gray, font=\sffamily\tiny, anchor=south] at (8.4,3.9) {reconstruct $\mathbf{U}$};

    % Legend
    \filldraw[accentpurple] (6.5,-0.6) circle (3pt);
    \node[anchor=west, font=\sffamily\footnotesize] at (6.7,-0.6) {$p$ at cell centre};
    \draw[->, secondarygreen, line width=1.6pt] (6.5,-1.1) -- (6.94,-1.1);
    \node[anchor=west, font=\sffamily\footnotesize] at (7.0,-1.1) {$U_n$ at face centre (primary)};

\end{tikzpicture}
\caption{Comparison of variable arrangements in the finite volume method. (a)~Colocated: all variables ($\mathbf{U}$, $p$) stored at cell centres; face velocities for flux computation require Rhie-Chow interpolation to suppress pressure--velocity decoupling (checkerboard oscillations). (b)~Hybrid staggered~\cite{Mahesh2004}: face-normal velocity $U_n$ is the primary kinematic variable stored directly at face centres; pressure $p$ at cell centres. Face flux is computed exactly from $U_n$ without interpolation, inherently preventing checkerboard modes. Cell-centred $\mathbf{U}$ is reconstructed from $U_n$ via least-squares each iteration.}
\label{fig:colocated-vs-staggered}
\end{figure}

\textbf{Colocated (standard FVM):} In the colocated arrangement used by \texttt{pimpleFoam} and \texttt{pimpleFoamPrecise}, all field variables ($\mathbf{U}$, $p$) are stored at cell centres. When computing the face flux for the continuity equation, cell-centred velocities must be interpolated to face centres. Naive linear interpolation leads to \emph{pressure--velocity decoupling}: the pressure field can develop a checkerboard pattern (alternating high/low values) that satisfies the discrete equations but is physically meaningless. The Rhie-Chow interpolation~\cite{Rhie1983} adds a pressure-dependent correction to the face velocity:
\[
    \mathbf{U}_f = \overline{\mathbf{U}}_f - \overline{\left(\frac{1}{A}\right)}_f
    \left[ (\nabla p)_f - \overline{(\nabla p)}_f \right]
\]
where overbars denote linear interpolation and $(\nabla p)_f$ is the face-normal gradient computed from the cell values on each side. This suppresses checkerboard modes but introduces an artificial dissipation term that depends on the mesh and timestep.

\textbf{Hybrid staggered~\cite{Mahesh2004}:} In \texttt{pimpleFoamStag}, the face-normal velocity $U_n = \mathbf{U} \cdot \hat{\mathbf{n}}$ is stored directly at face centres as the \emph{primary} kinematic variable. The face flux is then exactly $\phi_f = U_n \, |S_f|$---no interpolation, no Rhie-Chow correction. The pressure--velocity coupling is inherently stable because pressure and velocity unknowns live at staggered locations. The cell-centred velocity $\mathbf{U}$ (needed for the momentum equation, turbulence model, and post-processing) is reconstructed from $U_n$ each iteration via a least-squares procedure:
\[
    \mathbf{U}_P = \mathsf{T}_P^{-1} \sum_f U_{n,f} \, \mathbf{S}_f, \qquad
    \mathsf{T}_P = \sum_f \hat{\mathbf{n}}_f \otimes \mathbf{S}_f
\]

The pressure correction step is exact at the discrete level:
\[
    U_n = U_n^* - \left(\frac{1}{A}\right)_f \frac{\partial p}{\partial n}\bigg|_f
\]
where $U_n^*$ is the predicted face-normal velocity from the momentum equation projection and $\partial p / \partial n|_f$ is computed directly from adjacent cell pressures.

\subsection{Comparison of \texttt{pimpleFoamPrecise} and \texttt{pimpleFoamStag}}
\label{sec:solver-comparison}

\Cref{tab:solver-comparison} summarises the key differences between the two solvers.

\begin{table}[htbp]
\centering
\caption{Comparison of \texttt{pimpleFoamPrecise} and \texttt{pimpleFoamStag}}
\label{tab:solver-comparison}
\small
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Property} & \textbf{pimpleFoamPrecise} & \textbf{pimpleFoamStag} \\
\midrule
\tableheaderrow
Base algorithm & Standard PIMPLE (colocated) & PIMPLE with hybrid staggered grid \\
Primary velocity variable & Cell-centred $\mathbf{U}$ & Face-normal $U_n$ \\
\tablealternaterow
Pressure--velocity coupling & Rhie-Chow interpolation & Inherent (staggered grid) \\
Face flux computation & Interpolated from cell $\mathbf{U}$ & Exact: $\phi_f = U_n |S_f|$ \\
\tablealternaterow
Velocity correction & $\mathbf{U} = \mathbf{u}^* - (1/A)\,\nabla p$ & $U_n = U_n^* - (1/A)_f\,\partial p/\partial n$ \\
Reconstruction step & None & Least-squares $U_n \to \mathbf{U}$ each iteration \\
\tablealternaterow
Checkerboard risk & Suppressed by Rhie-Chow & Eliminated \\
Discrete mass conservation & Approximate (Rhie-Chow) & Exact \\
\tablealternaterow
HbyA registration & Yes (persistent field) & Yes (persistent field) \\
Ghost cell support & Yes (explicit overwrite) & Yes (explicit overwrite) \\
\tablealternaterow
Computational overhead & Negligible vs standard \texttt{pimpleFoam} & Small: one reconstruction per iteration \\
\midrule
\multicolumn{3}{@{}l}{\textbf{When to use}} \\
\midrule
Extraction phase & Recommended for \texttt{-preciseBC} & When staggered accuracy is needed \\
\tablealternaterow
Reconstruction phase & Use standard \texttt{pimpleFoam} & Use when exact mass conservation is critical \\
Ghost cell mode & Full support & Full support \\
\bottomrule
\end{tabularx}
\end{table}

\begin{tipbox}
For most users, \texttt{pimpleFoamPrecise} is the recommended choice for the extraction phase (it enables \texttt{-preciseBC} with minimal overhead). The reconstruction phase can use standard \texttt{pimpleFoam}. Use \texttt{pimpleFoamStag} when exact discrete mass conservation or elimination of Rhie-Chow artefacts is important, for example in acoustic simulations or highly under-resolved LES.
\end{tipbox}

\subsection{Ghost Cell Support}

Both \texttt{pimpleFoamStag} and \texttt{pimpleFoamPrecise} automatically detect ghost cell mode when a \texttt{ghostCells} cellZone exists in the mesh. When detected, the solvers:

\begin{enumerate}
    \item Overwrite ghost cell values for $\mathbf{U}$ and $p$ from extracted data at the \textbf{start} of each PIMPLE iteration (before momentum assembly)
    \item Overwrite again \textbf{after} the pressure correction loop (to prevent drift from equation solving)
\end{enumerate}

Ghost cell data is read from \texttt{constant/boundaryData/ghostCells/<timeName>/U} and \texttt{p}. The solvers handle \texttt{FoamFile} headers automatically.

\begin{notebox}
The explicit ghost cell overwrite in the solver is an alternative to the \texttt{ghostCellConstraint} fvOption. The solver-side overwrite applies at the field level (outside the equation system), while the fvOption applies at the matrix level via \texttt{eqn.setValues()}. Both achieve the same result; the solver-side overwrite is simpler and requires no additional configuration.
\end{notebox}

% ----------------------------------------------------------------------------
% GHOST CELL CONSTRAINT FVOPTION
% ----------------------------------------------------------------------------

\section{The \texttt{ghostCellConstraint} fvOption}
\label{sec:ghostCellConstraint}

The \texttt{ghostCellConstraint} is an \texttt{fvOption} that reads time-varying ghost cell data from disk and overrides the equation system in ghost cells using \texttt{eqn.setValues()}. It is instantiated as \texttt{scalarGhostCellConstraint} and \texttt{vectorGhostCellConstraint}.

\subsection{Configuration}

\begin{lstlisting}[language=OpenFOAM,caption={ghostCellConstraint fvOption configuration}]
// In constant/fvOptions
ghostU
{
    type            vectorGhostCellConstraint;
    selectionMode   cellZone;
...
\begin{table}[htbp]
\centering
\caption{ghostCellConstraint parameters}
\label{tab:ghost-constraint-params}
\begin{tabularx}{\textwidth}{@{}lllX@{}}
\toprule
\textbf{Parameter} & \textbf{Type} & \textbf{Required} & \textbf{Description} \\
\midrule
\texttt{cellZone} & word & Yes & Name of the ghost cell zone \\
\texttt{dataDir} & fileName & No & Path to boundaryData directory (default: \texttt{constant/boundaryData}) \\
\texttt{fields} & wordList & Yes & Fields to constrain \\
\bottomrule
\end{tabularx}
\end{table}
...
At each timestep, the fvOption:

\begin{enumerate}
    \item Reads ghost cell values from \texttt{<dataDir>/ghostCells/<timeName>/<fieldName>}
    \item Maps data indices to cell indices using \texttt{ghostCellMap} (written by \texttt{spaceTimeWindowInitCase} with the \texttt{-ghostCells} flag)
    \item Calls \texttt{eqn.setValues(cells, values)} to override the matrix rows for ghost cells
\end{enumerate}

The \texttt{setValues()} call replaces the matrix row for each ghost cell with a unit diagonal and moves the prescribed value to the source term, ensuring the linear solver produces exactly the prescribed value regardless of neighbouring cell contributions.

\begin{notebox}
For the pressure equation, the solver must call \texttt{fvOptions.constrain(pEqn)} after assembling the pressure matrix and before solving. Both \texttt{pimpleFoamStag} and \texttt{pimpleFoamPrecise} include this call. Standard \texttt{pimpleFoam} does not---if using the fvOption approach with standard \texttt{pimpleFoam}, add \texttt{fvOptions.constrain(pEqn)} to \texttt{pEqn.H}.
\end{notebox}
